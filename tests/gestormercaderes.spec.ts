...
describe('GestorMercaderes', () => {
  ...
  test('debería cargar mercaderes desde JSON al usar el array dummy por defecto', () => {
    ...
    if (databaseData && Array.isArray(databaseData) && databaseData.length > 0) {
      ...
      for (const mercader of almacenMap.values()) {
  ...
  test('debería lanzar error al agregar un mercader con ID duplicado', () => {
    ...
    const gestorMercaderes = new GestorMercaderes([mercader1]);
  ...
  test('debería eliminar un mercader con remove() y disminuir el tamaño', () => {
    ...
    const gestorMercaderes = new GestorMercaderes([mercader1, mercader2]);
