...
describe('Constructor Inventario', () => {
  test('Debe añadir y eliminar bienes correctamente', () => {
    ...
    // Agregamos un bien.
    const bien = new Bien(1, 'Espada de Acero', 'Espada forjada en acero', 'Acero', 3.5, 100);
  ...
  test('Debe calcular el stock correctamente a partir de transacciones', () => {
    ...
    const compra = new Transaccion(new Date(), [{ idBien: 1, cantidad: 10 }], 500, TipoTransaccion.Compra, 100);
    // Venta de 3 unidades.
    const venta = new Transaccion(new Date(), [{ idBien: 1, cantidad: 3 }], 150, TipoTransaccion.Venta, 200);
